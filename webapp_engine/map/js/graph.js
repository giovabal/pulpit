...
// max stays well below node sizes (1.5–15) so heavy edges are obvious without
// swamping the graph. EDGE_WEIGHT_BASE_SIZE is the fallback when a graph has no
// weight spread (e.g. the unweighted edge-weight strategy).
...
var THEMES = {
    dark: {
        label: 'Dark',
        canvas: 'rgba(17, 34, 51, 1)',
        fade: 'rgba(27, 44, 61, .75)',
        labelColor: '#ffffff',
        edgeOpacity: 0.25,
        curveEdges: true,
        ...
        cssVars: {
            ...
            '--c-fg-raw': '255, 255, 255',
            '--c-panel-bg': 'rgba(8, 24, 40, 0.55)',
            '--c-sidebar-bg': '#0d2035',
            '--c-sidebar-accent': '#a5b4fc',
            '--c-year-menu-bg': 'rgba(8, 24, 40, 0.96)',
            '--c-year-menu-shadow': 'rgba(0, 0, 0, 0.5)',
            '--c-accordion-arrow': 'invert(1)',
        },
    },
    light: {
        label: 'Light',
        canvas: 'rgba(240, 244, 248, 1)',
        fade: 'rgba(180, 195, 210, .75)',
        labelColor: '#1a2a3a',
        edgeOpacity: 0.30,
        curveEdges: true,
        defaultEdgeColor: '#aaaaaa',
        nodeBorderSize: 0.08,
        nodeBorderColor: 'rgba(255,255,255,0.8)',
        hoverFill: '#cddaeb',
        cssVars: {
            '--c-fg-raw': '20, 35, 60',
            '--c-panel-bg': 'rgba(220, 232, 246, 0.78)',
            '--c-sidebar-bg': '#dce8f4',
            '--c-sidebar-accent': '#4338ca',
            '--c-year-menu-bg': 'rgba(215, 230, 245, 0.97)',
            '--c-year-menu-shadow': 'rgba(0, 0, 0, 0.12)',
    ...
    minimal: {
        ...
        cssVars: {
            '--c-fg-raw': '30, 30, 30',
            '--c-panel-bg': 'rgba(248, 248, 248, 0.82)',
            '--c-sidebar-bg': '#f4f4f4',
            '--c-sidebar-accent': '#4f46e5',
            '--c-year-menu-bg': 'rgba(248, 248, 248, 0.97)',
            '--c-year-menu-shadow': 'rgba(0, 0, 0, 0.10)',
            '--c-accordion-arrow': 'none',
        },
    },
    print: {
        ...
        cssVars: {
            ...
            '--c-fg-raw': '0, 0, 0',
            '--c-panel-bg': 'rgba(255, 255, 255, 0.90)',
            '--c-sidebar-bg': '#ffffff',
            '--c-sidebar-accent': '#000000',
            '--c-year-menu-bg': 'rgba(255, 255, 255, 0.99)',
            '--c-year-menu-shadow': 'rgba(0, 0, 0, 0.15)',
            '--c-accordion-arrow': 'none',
        },
    },
};
...
function show_node_info(nodeId) {
    ...
    el('node_mutual_count').innerHTML = mutual.length;
    el('node_mutual_list').innerHTML = mutual.join('');
    el('node_in_count').innerHTML = inbound.length;
    el('node_in_list').innerHTML = inbound.join('');
    el('node_out_count').innerHTML = outbound.length;
    el('node_out_list').innerHTML = outbound.join('');
...
function apply_edge_widths() {
    ...
    var weights = graph.edges().map(function(edgeId) { return graph.getEdgeAttribute(edgeId, 'weight') || 0; });
    var scale = makeEdgeWidthScale(weights, EDGE_WEIGHT_MIN_SIZE, EDGE_WEIGHT_MAX_SIZE, EDGE_WEIGHT_BASE_SIZE);
    graph.edges().forEach(function(edgeId) {
        graph.setEdgeAttribute(edgeId, 'size', scale(graph.getEdgeAttribute(edgeId, 'weight') || 0));
    });
    sigma_instance.refresh();
}
...
function animate_year_transition(new_pos_data, new_ch_data, duration_ms) {
    ...
    new_only.forEach(function(id) {
        ...
        graph.addNode(id, Object.assign({}, m, {
            ...
            color: c,
            originalColor: c,
            borderSize: _yt.nodeBorderSize,
            borderColor: _yt.nodeBorderColor,
        }));
    });
        ...
        if (raw < 1) {
            ...
            requestAnimationFrame(function() {
                ...
                new_pos_data.edges.forEach(function(edge) {
                    ...
                    delete attrs.target;
                    delete attrs.id;
                    try { graph.addEdge(edge.source, edge.target, attrs); } catch (err) { if (typeof console !== 'undefined' && console.warn) console.warn('addEdge skipped', edge.source, '→', edge.target, err.message); }
                });
