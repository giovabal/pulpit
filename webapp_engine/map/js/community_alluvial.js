...
function _reorderByNeighbor(stages, flows, target, neighbor, forward) {
    var tStage = stages[target],
        nStage = stages[neighbor];
    var nPos = {};
...
function _ribbonInfo(srcStage, dstStage, r, srcHex, dstHex) {
    ...
    return {
        srcLabel: r.src,
        dstLabel: r.dst,
        srcCol: srcStage.colLabel,
        dstCol: dstStage.colLabel,
        srcHex: srcHex,
        dstHex: dstHex,
        channels: channels,
        srcTotal: srcTotal,
        dstTotal: dstTotal,
        srcPct: _pct(cnt, srcTotal),
        dstPct: _pct(cnt, dstTotal),
    };
}
...
function _render_stages_svg(stages, availWidth, ariaLabel, onSelect) {
    ...
    for (var bi = 0; bi < stages.length - 1; bi++) {
        ...
        _boundaryRibbons(flows[bi], src, dst, unit).forEach(function(r) {
            var sc = src.byLabel[r.src],
                dc = dst.byLabel[r.dst];
            if (!sc || !dc) return;
            ...
            path.addEventListener("mouseenter", function(e) {
                if (path !== _selected) path.setAttribute("fill-opacity", "0.7");
                showTip(e, tip);
            });
            path.addEventListener("mousemove", moveTip);
            path.addEventListener("mouseleave", function() {
                path.setAttribute("fill-opacity", path === _selected ? RIBBON_SELECTED_OPACITY : RIBBON_OPACITY);
                hideTip();
            });
            ...
            path.addEventListener("keydown", function(e) {
                if (e.key === "Enter" || e.key === " ") {
                    e.preventDefault();
                    _selectRibbon(path, info, key, false);
                }
            });
    ...
    stages.forEach(function(st, si) {
        ...
        st.comms.forEach(function(c) {
            ...
            if (c.h >= LABEL_MIN_H) {
                ...
                var txt = _svgEl("text", {
                    x: isFirst ? c.x - 5 : c.x + BOX_W + 5,
                    y: c.y + c.h / 2,
                    "text-anchor": isFirst ? "end" : "start",
...
function _metaSep() {
    ...
    s.className = "channel-row-meta-sep";
    s.textContent = "·";
    return s;
}
...
function _channelCard(ch, node, fallbackHex) {
    ...
    if (node) {
        ...
        meta2.className = "channel-row-meta";
        meta2.appendChild(_metaItem("bi-chat-left-text", fmtInt(node.messages_count || 0)));
        if (node.fans) {
            meta2.appendChild(_metaSep());
            meta2.appendChild(_metaItem("bi-people", fmtInt(node.fans)));
        }
        var start = _ymShort(node.activity_start);
        if (start) {
            meta2.appendChild(_metaSep());
            meta2.appendChild(_metaItem("bi-calendar-range", start + " – " + (_ymShort(node.activity_end) || "now")));
        }
...
function _renderChannelsPanel(panel, info, nodeById) {
    ...
    head.className = "alluvial-channels-head";
    var titleEl = document.createElement("div");
    titleEl.className = "alluvial-channels-title";
    ...
    function commTag(label, col, hex) {
        ...
        colEl.className = "text-muted";
        colEl.textContent = "[" + col + "]";
        titleEl.appendChild(sw);
        titleEl.appendChild(strong);
        titleEl.appendChild(colEl);
    }
    commTag(info.srcLabel, info.srcCol, info.srcHex);
    var arrow = document.createElement("span");
    arrow.className = "alluvial-channels-arrow";
    arrow.textContent = "→";
    ...
    cnt.textContent = "· " + n + " channel" + (n === 1 ? "" : "s");
    titleEl.appendChild(cnt);
    if (info.srcTotal && info.dstTotal) {
        var share = document.createElement("span");
        share.className = "text-muted";
        // The flow as a fraction of each community it connects: n/m of the source (left) and n/p of the
        // destination (right), with the percentage in parentheses. Same numerator n (this flow's channels),
        // different denominators (the two community sizes).
        share.textContent = "· " + n + "/" + info.srcTotal + " (" + info.srcPct + ") ↔ " +
            n + "/" + info.dstTotal + " (" + info.dstPct + ")";
        share.title = "This flow is " + n + "/" + info.srcTotal + " of " + info.srcLabel + " [" + info.srcCol + "] (" +
            info.srcPct + ") and " + n + "/" + info.dstTotal + " of " + info.dstLabel + " [" + info.dstCol + "] (" +
            info.dstPct + ").";
    ...
    close.type = "button";
    close.className = "alluvial-channels-close";
    close.setAttribute("aria-label", "Close channel list");
    close.textContent = "×";
    head.appendChild(close);
    panel.appendChild(head);

    var grid = document.createElement("div");
    grid.className = "alluvial-channels-grid";
...
function _buildFigure(stages, availWidth, ariaLabel, nodeById, initialKey) {
    ...
    panel.className = "alluvial-channels";
    panel.hidden = true;
