// Shared renderer for the equivalence heatmaps (structural equivalence and
// behavioural equivalence). Both consume the same JSON schema
// `{ node_ids, node_labels, measures, note, cells_lower }` and differ only in the
// data file, the container id, and the method note carried in the JSON.
...
// ── Main render ────────────────────────────────────────────────────────────────
function _render(containerId, simData, channelData, communities, meta, sortMode, sortMeasureKey, stratKey) {
    ...
    sortSel.addEventListener("change", function() {
        ...
        if (val === "community") {
            ...
            _render(containerId, simData, channelData, communities, meta, "community", null, stratSel.value || null);
        } else {
            stratWrap.style.display = "none";
            _render(containerId, simData, channelData, communities, meta, "measure", val.replace("measure:", ""), null);
        }
    });
...
export function initEquivalenceMatrix(opts) {
    ...
    Promise.all([
        ...
        _render(containerId, results[0], results[1], results[2], results[3], "community", null, null);
    }).catch(function(err) {
