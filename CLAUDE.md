...
| `MODULEROLE` | Guimerà & Amaral (2005) within-module role: emits numeric `within_module_z` (within-community degree z-score) + categorical `module_role` label (7 roles from the z/P plane). Dispatched specially (needs a community partition); basis auto-resolved from the labels already on the nodes (bridging basis if set, else `leiden_directed`, else first available). `module_role` is **not** in `measures_labels` — it is exported as a string column like `organization` (channels.json node_keys, CSV, XLSX, a "Role" column in `channel_table.js`); `within_module_z` flows the normal numeric measure path |
...
Edge weight = (forwards + references) / total messages from source channel. **Direction is fixed: a forward of source Y's content by amplifier X produces an X→Y edge (the citation convention — citing → cited). `Channel.in_degree` therefore counts how many channels cite/forward this one (audience); `out_degree` counts how many channels it cites/forwards (curatorial activity). Measures whose academic definition runs the other way — SIR `SPREADING` (Kitsak et al. 2010) and `TROPHICLEVEL` (MacKay 2020) — reverse the graph internally so content cascades flow source→amplifier; every other measure (PageRank, HITS, betweenness/harmonic, Burt constraint, bridging, vacancy PPR, ORGANIZATION/Leiden communities, k-core) treats edges in the as-built citation direction.**
