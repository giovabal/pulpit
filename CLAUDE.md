...
- **`network/community.py`** — Community detection: ORGANIZATION, LABELPROPAGATION, LEIDEN, LEIDEN_DIRECTED, LEIDEN_CPM, KCORE. (The flow / random-walk detectors INFOMAP, INFOMAP_MEMORY, MCL, WALKTRAP, STRONGCC were removed under the same one-degree rationale as the path/flow measures: modularity is a single-step walk = one degree, while those integrate many steps.) `LEIDEN_CPM` (resolution γ) is **parameterised** and may be requested more than once with different parameters; `parse_strategies` turns `--community-strategies` into `StrategyInstance` objects (mirroring measures), each mapping to a parameter-suffixed partition key (`StrategyInstance.key`, e.g. `leiden_cpm_resolution_0_05`; just `leiden_directed` for parameter-free strategies). `canonical_strategy_key` strips the suffix; `strategy_display_label` renders the human label. (`LEIDEN_CPM` replaces the old fixed `LEIDEN_CPM_COARSE` / `LEIDEN_CPM_FINE` presets.)
