...
- **`network/community.py`** — Community detection: ORGANIZATION, LABELPROPAGATION, LEIDEN, LEIDEN_DIRECTED, LEIDEN_CPM, LOUVAIN, KCORE, SBM. (The flow / random-walk detectors INFOMAP, INFOMAP_MEMORY, MCL, WALKTRAP, STRONGCC were removed under the same one-degree rationale as the path/flow measures: modularity is a single-step walk = one degree, while those integrate many steps.) **LOUVAIN** (`detect_louvain`, Blondel et al. 2008) is the classic modularity baseline, re-added unselected-by-default purely for comparability with the large body of older studies that report Louvain partitions — it optimises the same modularity objective on the same symmetrised W+Wᵀ graph as LEIDEN, which supersedes it (Leiden adds the connectivity-guarantee refinement); so it is in `UNDIRECTED_BASIS_STRATEGIES`, a genuine partition (joins NMI/consensus, *not* in `_NMI_EXCLUDED_STRATEGIES`), parameter-free, and `seed=0`-reproducible. Prefer LEIDEN / LEIDEN_DIRECTED for new analyses. `LEIDEN_CPM` (resolution γ) and `SBM` (`mode=FLAT|NESTED`) are **parameterised** and may be requested more than once with different parameters; `parse_strategies` turns `--community-strategies` into `StrategyInstance` objects (mirroring measures), each mapping to a parameter-suffixed partition key (`StrategyInstance.key`, e.g. `leiden_cpm_resolution_0_05`, `sbm_mode_nested`; just `leiden_directed` for parameter-free strategies). `canonical_strategy_key` strips the suffix; `strategy_display_label` renders the human label. (`LEIDEN_CPM` replaces the old fixed `LEIDEN_CPM_COARSE` / `LEIDEN_CPM_FINE` presets.) **SBM** is a directed, degree-corrected Bayesian stochastic block model (Karrer & Newman 2011; Peixoto 2014/2017) fitted via `graph-tool` (`detect_sbm`, lazy import → clear `ValueError` if the package is absent; it is **not** pip-installable — conda-forge / apt only). Its blocks are *citation-role / structural-equivalence classes* (Lorrain & White 1971), **not** cohesive communities — it recovers disassortative / core-periphery / bipartite source-amplifier structure modularity is blind to; **unweighted** (binary citation structure) and seeded for reproducibility. SBM is directed, so it is *not* in `UNDIRECTED_BASIS_STRATEGIES` (reported with directed modularity) and *not* in `community_stats._NMI_EXCLUDED_STRATEGIES` (it is a genuine partition, unlike the KCORE shell decomposition, so it joins the NMI/consensus comparison).
