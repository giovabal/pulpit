...
| Path | Description | Built-in default |
| :--- | :---------- | ---------------: |
| `vacancy.measures` | Vacancy-succession algorithms. Available: `AMPLIFIER_JACCARD`, `NEW_ADOPTERS`, `STRUCTURAL_EQUIV`, `BROKERAGE`, `ORIGIN_OVERLAP`, `TEMPORAL`. Empty list disables vacancy analysis. See [Vacancy analysis](vacancy-analysis.md). | `[]` |
...
Resistance to node removal: residual-size R-index per attack strategy, z-score + BH-adjusted empirical p against a null model, intra/inter community edge survival, ban-wave scenarios per partition, an optional backbone-sensitivity sweep, and an optional historical ban-replay validation. See [Robustness analysis](robustness-analysis.md).
...
| `robustness.null` | Number of null-model simulations per strategy; `0` disables the null model (no z-scores / p-values). The empirical p-value's floor is `2/(K+1)` — the default 20 cannot certify α = 0.05; use 79+ (e.g. 100) for publication-grade claims | `20` |
| `robustness.null_model` | Which null to rewire against: `configuration` (degree + strength preserving) or `reciprocal` (also preserves each node's reciprocated degree and global reciprocity — Squartini & Garlaschelli 2011) | `configuration` |
| `robustness.strategies` | Attack strategies. Static: `random`, `in_strength`, `out_strength`, `pagerank`, `betweenness`, `collective_influence`, `subscribers`. Dynamic (re-rank per removal): `in_strength_dyn`, `out_strength_dyn`, `pagerank_dyn`, `betweenness_dyn`, `collective_influence_dyn`, `fragmentation_dyn`. Use `["ALL"]` for every strategy. | `[]` |
...
| `robustness.alpha_grid` | List of disparity-filter α values for the backbone-sensitivity sweep (R recomputed per strategy×metric at each; `0` = full graph). Empty skips the sweep | `[]` |
| `robustness.replay` | Run the historical ban-replay validation (needs `timeline.step = year` and recorded `ChannelVacancy` closures) | `false` |
