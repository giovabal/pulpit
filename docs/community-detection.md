...
*Walktrap measures how similar two channels' short random walks are: channels embedded in the same dense neighbourhood end up close together, so they are grouped even when no direct edge links them.*

Walktrap operationalises the intuition that a short random walk on a graph tends to stay inside its starting community. For each node u it builds the probability distribution P_u over where a walker of fixed length t (Pulpit uses t=4, the library default) ends up; two nodes are similar when their distributions are close in a degree-weighted L² distance. Ward's agglomerative clustering then merges nodes/clusters bottom-up by minimum distance into a full dendrogram, which is cut at the level that maximises Newman-Girvan modularity. The distinctive property — the reason to pick Walktrap over `LOUVAIN`/`LEIDEN` — is that two channels with **no direct edge** between them can still cluster together when their walk distributions converge on the same neighbours: shared-neighbourhood structure, not just direct citation density, drives the partition.

Pulpit builds the igraph from the W+Wᵀ undirected projection of the citation graph (`to_undirected_sum`, same projection used by `LEIDEN`, `LOUVAIN`, `LABELPROPAGATION`, and `KCORE`): reciprocal forwards are collapsed to one edge whose weight is the sum of the two directional weights (`w(u→v) + w(v→u)`), and citation direction is therefore not preserved. Walk length is fixed at the library default `steps=4`. Edge weights are honoured (`community_walktrap` accepts weighted random walks), so the chosen `--edge-weight-strategy` shapes both the walk distributions and the modularity cut. The dendrogram is cut by `.as_clustering()` at the modularity-maximising level — Walktrap therefore inherits the Fortunato-Barthélemy resolution limit *at the cut step*, even though the underlying random-walk distance has no such limit. Pulpit consumes only the final flat partition: the full dendrogram is built internally by igraph but is **not** exposed in any export (JSON, GEXF, GraphML, or community tables) and is not visualised in the web interface. The algorithm is fully deterministic given the input weights — no random seed.

**References:**
- Pons, P. & Latapy, M. (2005) "Computing communities in large networks using random walks." *ISCIS 2005, Lecture Notes in Computer Science* 3733. [doi:10.1007/11569596_31](https://doi.org/10.1007/11569596_31) — original conference paper with the random-walk distance definition and the t=4 step-length recommendation that Pulpit inherits via igraph's default.
- Pons, P. & Latapy, M. (2006) "Computing communities in large networks using random walks." *Journal of Graph Algorithms and Applications* 10(2), 191–218. [doi:10.7155/jgaa.00124](https://doi.org/10.7155/jgaa.00124) — extended journal version with the full formal analysis of the Ward-based merging and the modularity cut.

**In practice:** Walktrap is the right tool when you suspect channels in the same community are linked **transitively through shared sources** rather than by direct citations. A common Telegram pattern: many small amplifiers that all forward from the same set of larger sources without forwarding each other — Walktrap groups them by their shared walk targets, while strict modularity-based methods may scatter them because their pairwise edge density is low. The modularity cut at the end means Walktrap inherits the resolution limit of `LOUVAIN`/`LEIDEN`, so small dense groups may still be absorbed into bigger ones; for that case prefer `LEIDEN_CPM_FINE` or `MCL`. Run Walktrap alongside `LEIDEN` and read the disagreement in the consensus matrix: pairs co-clustered by Walktrap *but not* by Leiden are channels linked by shared neighbourhood structure rather than mutual citation — the disagreement itself is the signal.

**Example.** A monitoring project tracks 220 channels in an anti-vaccine ecosystem. `LEIDEN` returns six communities largely aligned with the analyst's regional groupings. `WALKTRAP` returns five — one of which fuses two of Leiden's smaller regional groupings into a single cluster. Inspection shows the two fused groupings share the same handful of international "scientific authority" channels they all forward from, but rarely forward each other directly. Walktrap recognises them as a single shared-neighbourhood cluster because a short random walk from any member of either grouping converges on the same external sources; Leiden splits them because their pairwise edge density is too low to clear the modularity threshold. The disagreement is the actionable signal — the two regional groupings are operationally distinct producers tapping a shared source pool.

---

## Label propagation

*Label propagation finds communities by spreading labels through the graph until every node carries the most-frequent label among its neighbours — no parameters, no quality function, no weights, near-linear time per iteration.*

Each node is initialised with a unique label. At each iteration every node adopts the label that occurs most often among its neighbours; ties — when several labels are tied for first place — are resolved by the **Prec-Max** rule (Cordasco & Gargano 2010): the current label is kept if it sits among the top labels, otherwise the lexicographically largest top label wins. The process terminates when no node would change its label on the next sweep, and nodes sharing a label at convergence form a community. The algorithm has no global quality function to maximise — communities emerge from purely local consensus rather than from a network-wide objective — which is what makes it dramatically faster than modularity- or flow-based detectors but also far more sensitive to small changes in the graph.

Pulpit calls the **semi-synchronous variant** of Cordasco & Gargano (2010), not the original asynchronous algorithm of Raghavan, Albert & Kumara (2007). The semi-synchronous variant first greedy-colours the graph so neighbouring nodes get different colours, then updates all same-coloured nodes simultaneously in each sweep — the two safeguards needed to make label propagation provably terminate and avoid the bipartite-style oscillations the asynchronous version suffers from. Combined with the deterministic Prec-Max tie-break, the result is a **fully deterministic** partition: rerunning on the same graph produces the same communities, no random seed required.

Pulpit symmetrises the citation graph with `to_undirected_sum` (W+Wᵀ — reciprocal forwards collapsed to one edge of summed weight, the same projection used by `LEIDEN`, `LOUVAIN`, `WALKTRAP`, and `KCORE`) before running label propagation. The choice of projection is for **consistency with the other undirected detectors**, not for substantive effect: NetworkX's `label_propagation_communities` rejects directed input and silently **ignores edge weights**, so the partition would be identical under a plain `to_undirected()` and is **invariant to `--edge-weight-strategy`** — `NONE`, `TOTAL`, `PARTIAL_MESSAGES`, and `PARTIAL_REFERENCES` all yield the same communities. Citation direction is also discarded: an X→Y forward and a Y→X forward contribute the same way.

**References:**
- Raghavan, U.N., Albert, R. & Kumara, S. (2007) "Near linear time algorithm to detect community structures in large-scale networks." *Physical Review E* 76(3), 036106. [doi:10.1103/PhysRevE.76.036106](https://doi.org/10.1103/PhysRevE.76.036106) — the original asynchronous algorithm, the foundational reference for the label-spreading idea.
- Cordasco, G. & Gargano, L. (2010) "Community detection via semi-synchronous label propagation algorithms." *2010 IEEE International Workshop on Business Applications of Social Network Analysis (BASNA)*. [doi:10.1109/BASNA.2010.5730298](https://doi.org/10.1109/BASNA.2010.5730298) — the semi-synchronous variant with colour-class scheduling and the Prec-Max tie-break, the version NetworkX (and therefore Pulpit) actually runs.

**In practice:** Label propagation is the cheapest community detector in Pulpit's suite — orders of magnitude faster than `INFOMAP`, `MCL`, or `WALKTRAP` — which makes it the right choice for very large networks where the more sophisticated methods are too slow, or as an extremely cheap **parameter-free baseline** in the consensus matrix. Because it ignores both edge weights and direction, it sees only the coarsest possible view of the citation graph — pure unweighted connectivity. Read this property both ways:

- as a *strength*: a grouping that appears under both `LABELPROPAGATION` and `LEIDEN` (or `LEIDEN_DIRECTED`) is unlikely to be an artefact of weighting, edge direction, or a resolution-parameter choice — it is robust on the bare topology alone, and that is genuine signal in the consensus matrix.
- as a *limitation*: label propagation cannot distinguish a heavy reciprocal tie from a light one, nor a broadcast hub from one of its amplifiers — every structural distinction Pulpit exposes through `--edge-weight-strategy` or directed-modularity-style nulls is invisible to it. It is also known to produce **a single giant community absorbing most of the network** when the graph is dense enough for the consensus dynamic to run away — a well-documented failure mode noted already in Raghavan et al. 2007. A label-propagation run that returns one dominant cluster is a signal to switch to `LEIDEN`, not evidence that the network has no substructure.

Use `LABELPROPAGATION` as a fast robustness check alongside `LEIDEN`/`LEIDEN_DIRECTED`, not as a primary detector when weight or direction carries the meaning.

**Example.** A monitoring project tracks 1,200 channels. `LEIDEN_DIRECTED` takes several minutes and returns 14 communities; `LABELPROPAGATION` runs in well under a second and returns 8. Six of the eight label-propagation communities each absorb a pair of `LEIDEN_DIRECTED` communities; the remaining two correspond one-to-one with Leiden communities. Inspection reveals that the merged pairs are exactly the hub-and-spoke patterns where one aggregator is heavily cited by a constellation of small amplifiers without citing them back — `LEIDEN_DIRECTED` splits the broadcaster from its audience because the directed null model penalises the asymmetry, whereas label propagation, blind to direction, sees one densely-connected blob. The eight label-propagation communities are the robust **direction-agnostic** substructure (everyone agrees these channels belong together at all); the six Leiden splits inside them are the **direction-driven refinement** label propagation cannot see — and the disagreement itself is the actionable signal, separating "shared neighbourhood" from "shared editorial role".

---

## Weakly Connected Components (WEAKCC)

*Two channels belong to the same weakly connected component when at least one chain of edges connects them — regardless of which way each edge points.*

A WCC is the maximal set of nodes mutually reachable once edge direction is dropped: on the directed graph G = (V, E) it is an equivalence class of V under the symmetric closure of "u and v share an edge". The decomposition is parameter-free, weight-blind, and direction-blind, and is computed in O(|V| + |E|) by a single BFS sweep over the symmetrised graph — Hopcroft & Tarjan (1973) gave the canonical linear-time algorithm that NetworkX's `weakly_connected_components` implements. WEAKCC is a **structural decomposition**, not a community-detection method in the modularity / map-equation / random-walk sense: it carries no notion of "more densely connected than expected", only whether any chain of edges exists at all.

The WCC/SCC pair is the standard macro-structural lens for large directed networks — Broder et al. (2000) used it to draw the "bow-tie" picture of the World Wide Web, with WCC bounding the reachable universe and SCC carving the mutually-reachable core out of it. On most empirical networks the giant WCC absorbs nearly every node and a long tail of small satellite components is left behind; the size of the giant component versus the rest is itself the headline statistic, reported in Pulpit's [whole-network table](whole-network-statistics.md#components) as `WCC count` and `Largest WCC fraction`.

Pulpit's `detect_weakcc` calls `nx.weakly_connected_components(graph)` on the as-built citation DiGraph, sorts the components by size (largest first), and labels every channel with its component id — community 1 is the largest WCC, community 2 the next-largest, and so on. The partition is **unweighted** (membership is a pure 0/1 reachability question) and therefore invariant to `--edge-weight-strategy`, and **direction-invariant** by construction. Truly isolated channels — no edges in either direction — become singleton components and keep individual labels; no isolated-node bundling runs (`merge_isolated=False`), because every isolated node already *is* its own WCC.

Because WEAKCC is structural rather than community-like, Pulpit deliberately **excludes it from the consensus matrix and the NMI matrix** (alongside `STRONGCC` and `KCORE`): the giant component co-assigns almost every channel pair, which would otherwise inflate apparent agreement across detection strategies. Modularity is still reported (against the undirected W+Wᵀ null, matching the direction-blind basis on which the partition was built), but it should not be read as partition quality — for WEAKCC the meaningful quality story is the count and size of the components themselves, not whatever Q happens to fall out.

**References:**
- Hopcroft, J. & Tarjan, R. (1973) "Algorithm 447: efficient algorithms for graph manipulation." *Communications of the ACM* 16(6), 372–378. [doi:10.1145/362248.362272](https://doi.org/10.1145/362248.362272) — the linear-time BFS/DFS algorithm NetworkX implements for connected-component discovery.
- Broder, A. et al. (2000) "Graph structure in the Web." *Computer Networks* 33(1–6), 309–320. [doi:10.1016/S1389-1286(00)00083-9](https://doi.org/10.1016/S1389-1286(00)00083-9) — the canonical use of WCC together with SCC to characterise the macro-structure of large directed networks (the "bow-tie" decomposition Pulpit's `WEAKCC` / `STRONGCC` pair mirrors).

**In practice:** treat `WEAKCC` as a **diagnostic for structural disconnection**, not as a community label. If it returns one giant component covering nearly all channels (the usual case on a cohesive monitoring project), the citation network is one connected ecosystem and the partition is uninformative for grouping — switch to `LEIDEN` / `LEIDEN_DIRECTED` for the substructure inside. If it returns several large components of comparable size, the project is straddling structurally unrelated ecosystems with no shared citation chain at all — a finding worth surfacing in its own right, because no modularity-, flow-, or walk-based detector can ever produce a community that crosses a WCC boundary (no path exists to support it). The companion network-level metrics `WCC count` and `Largest WCC fraction` carry the same information at summary scale and are usually where the WEAKCC result is read; the per-channel labels are useful mainly for inspecting which channels fall into each satellite component.

**Example.** A monitoring project covering two politically unrelated media ecosystems — domestic far-right channels and a foreign-language diaspora cluster — produces a graph with `WCC count = 2` and `Largest WCC fraction ≈ 0.62`. `WEAKCC` labels every channel as either community 1 (the domestic ecosystem, the larger one) or community 2 (the diaspora). No `LEIDEN`, `INFOMAP`, or `MCL` partition can ever merge the two: there is no path between them, weighted or unweighted, in either direction. The two ecosystems are genuinely structurally isolated — they may share topics, but they do not share citation chains. The next move is either to inspect the diaspora component on its own (intentional scope, or crawl artefact?) or to re-scope the crawler to find the missing bridge channels.

---

## Strongly Connected Components (STRONGCC)

*Two channels belong to the same strongly connected component only when there is a directed path from A to B **and** a directed path from B to A — they must mutually reach each other by following actual citation arrows, not after dropping direction.*

An SCC is the maximal set of nodes mutually reachable on the directed graph G = (V, E) — an equivalence class of V under the symmetric relation "u reaches v *and* v reaches u". The SCCs partition V on any directed graph; the quotient graph that collapses each SCC to a single node is the **condensation**, which is by construction a DAG (every directed cycle lives entirely inside one SCC). The decomposition is parameter-free, weight-blind, and direction-aware, and is computed in O(|V| + |E|) by Tarjan's (1972) iterative depth-first search — the algorithm NetworkX's `strongly_connected_components` implements. STRONGCC is a **structural decomposition**, not a community-detection method in the modularity / map-equation / random-walk sense: it carries no notion of "more densely connected than expected", only whether a closed directed cycle of any length passes through the pair at all.

The WCC/SCC pair is the standard macro-structural lens for large directed networks — Broder et al. (2000) used it to draw the "bow-tie" picture of the World Wide Web, in which the SCC is the central knot whose nodes mutually reach each other, with an IN tendril of nodes that reach the SCC without being reached back and an OUT tendril of nodes the SCC reaches without ever getting back from. On Pulpit's amplifier→source citation graph, a non-trivial SCC requires reciprocal citation chains: at minimum two channels each forwarding (or `t.me/`-referencing) the other; more typically a closed loop A → B → C → … → A formed by transitive reciprocity. SCCs of size ≥ 2 are the strict-reciprocity cores; the long tail is singleton SCCs — isolated nodes plus channels that only ever cite or are only ever cited.

Pulpit's `detect_strongcc` calls `nx.strongly_connected_components(graph)` directly on the as-built citation DiGraph, sorts the components by size (largest first), and labels every channel with its component id — community 1 is the largest SCC, community 2 the next-largest, and so on. The partition is **unweighted** (membership is a pure 0/1 mutual-reachability question) and therefore invariant to `--edge-weight-strategy`, and **direction-aware** by construction: switching `G` for `G.reverse()` produces the same partition (reversing every edge preserves mutual reachability) but **symmetrising** the graph would collapse the SCC partition all the way to the [WCC](#weakly-connected-components-weakcc) partition. Truly isolated channels — no edges in either direction — become singleton components and keep individual labels; no isolated-node bundling runs (`merge_isolated=False`), because every isolated node already *is* its own trivial SCC.

Because STRONGCC is structural rather than community-like, Pulpit deliberately **excludes it from the consensus matrix and the NMI matrix** (alongside `WEAKCC` and `KCORE`): on sparse citation graphs the long singleton tail dominates the partition and would inflate apparent disagreement between detectors, while a single dominant SCC on dense ones would inflate apparent agreement — neither is informative as a community-detection signal. Modularity is still reported (against the directed null model `kᵢᵒᵘᵗ · kⱼⁱⁿ / m` of Leicht & Newman 2008, matching the direction-aware basis on which the partition was built), but it should not be read as partition quality — for STRONGCC the meaningful quality story is the size of the largest SCC and the size distribution of the rest, not whatever Q happens to fall out. The same SCC structure also feeds two adjacent measurements: the [`SCC count` and `Largest SCC fraction`](whole-network-statistics.md#components) headline statistics in the whole-network table, and the [`R_scc` robustness curve](robustness-analysis.md#r_scc--strongly-connected-component) that tracks how the largest SCC shrinks under targeted node removal.

**References:**
- Tarjan, R. (1972) "Depth-first search and linear graph algorithms." *SIAM Journal on Computing* 1(2), 146–160. [doi:10.1137/0201010](https://doi.org/10.1137/0201010) — the linear-time DFS algorithm NetworkX's `strongly_connected_components` implements for SCC discovery.
- Broder, A. et al. (2000) "Graph structure in the Web." *Computer Networks* 33(1–6), 309–320. [doi:10.1016/S1389-1286(00)00083-9](https://doi.org/10.1016/S1389-1286(00)00083-9) — the canonical use of SCC together with WCC to characterise the macro-structure of large directed networks (the "bow-tie" decomposition Pulpit's `STRONGCC` / `WEAKCC` pair mirrors).
- Leicht, E.A. & Newman, M.E.J. (2008) "Community structure in directed networks." *Physical Review Letters* 100, 118703. [doi:10.1103/PhysRevLett.100.118703](https://doi.org/10.1103/PhysRevLett.100.118703) — the directed modularity null model under which Pulpit reports `Q` for the SCC partition.

**In practice:** treat `STRONGCC` as a **strict-reciprocity diagnostic**, not as a general community label. On Pulpit's amplifier→source citation graph, a large non-trivial SCC is the operationalisation of "coordinated nucleus" or "echo chamber" at its strictest — a group of channels that all mutually cite each other along closed directed cycles, not merely densely co-cited or sharing common sources. Read the result in conjunction with [`INFOMAP`](#infomap) (which surfaces flow-confined modules — closer to the cognitive sense of echo chamber but without strict reciprocity) and [`LEIDEN_DIRECTED`](#leiden-directed) (which surfaces dense directional brokerage but tolerates one-way ties): a grouping that is *also* an SCC is the strict mutual-citation core inside the flow trap. On most empirical Telegram networks the answer is either one or a few large SCCs (the coordinated centre, sometimes plus a small reciprocal pocket or two) embedded in a long tail of singletons (the periphery of one-way amplifiers and one-way sources). If every channel is a singleton, the network has no reciprocal cycles at all — either a pure broadcast tree, a project at very early crawl stage, or a structurally tree-like ecosystem with no closed citation loops.

**Example.** A monitoring project tracks 320 channels in a coordinated disinformation operation. `STRONGCC` returns one component of size 12 plus 308 singletons (`Largest SCC fraction ≈ 0.038` in the whole-network table). Inspection of the 12-node SCC reveals a tight coordination ring: each member forwards every other member's content within hours of publication, and at least one closed cycle A → B → C → … → A can be traced through every pairing — the textbook pattern for manufactured organic consensus. The 308 singletons are the downstream amplifiers (one-way forwarders of the ring's content) plus a handful of upstream sources the ring quotes but never gets quoted by. `INFOMAP` independently identifies the same 12 channels as one flow module of nine plus a flow module of three; `LEIDEN_DIRECTED` merges them into a larger 31-channel community that also pulls in the most active one-way amplifiers. The 12-channel intersection — present under all three lenses *and* a single SCC — is the actionable target: removing those 12 in the [`R_scc`](robustness-analysis.md#r_scc--strongly-connected-component) attack collapses the largest SCC to size 1 in twelve steps, confirming the ring is the sole structural keystone of the operation's reciprocity.
