...
- **`post_author` stored per message** — `Message` gains a `post_author CharField(max_length=255, blank=True)`, populated from `telegram_message.post_author` during both initial crawl and the stats-refresh pass. When set, the author's name appears on post cards below the message body (using the same pen icon as the channel-level "Signed posts" badge), completing the per-post counterpart to the existing channel metadata.
- **`edit_date` stored per message** — `Message` gains an `edit_date DateTimeField(null=True)`, populated from `telegram_message.edit_date` during both the initial crawl and the stats-refresh pass. When set, post cards in channel detail pages and message search results display a pencil icon and "Edited" label alongside the original post date (exact edit timestamp shown on hover). Useful for disinformation research: edits made after initial amplification can change a message's meaning while preserving its original reach.
