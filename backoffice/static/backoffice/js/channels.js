(function() {
    ...
    function _buildQueryString() {
        ...
        var page = Math.floor(_offset / PAGE_SIZE) + 1;
    ...
    function renderTable(channels) {
        ...
        channels.forEach(function(ch) {
            ...
            tdId.className = "bo-td--num bo-td--id";
            var idLink = document.createElement("a");
            idLink.href = "/manage/channels/" + ch.id + "/";
            ...
            tdName.className = "bo-ch-cell";
            var nameWrap = document.createElement("div");
            nameWrap.className = "bo-ch-name-wrap";
            var nameEl = document.createElement("div");
            nameEl.className = "bo-ch-name";
            nameEl.textContent = ch.title || ("ID " + ch.id);
            nameWrap.appendChild(nameEl);
            if (ch.username) {
                var unEl = document.createElement("a");
                unEl.className = "bo-ch-username";
                unEl.href = "https://t.me/" + ch.username;
                unEl.target = "_blank";
                unEl.rel = "noopener noreferrer";
            ...
            if (picEl) {
                ...
                picEl.addEventListener("click", function(e) {
                    e.stopPropagation();
                    _openPicModal(ch);
                });
            ...
            tdSub.className = "bo-td--num";
            tdSub.textContent = fmtInt(ch.participants_count);
            tr.appendChild(tdSub);

            /* in-degree */
            var tdIn = document.createElement("td");
            tdIn.className = "bo-td--num";
            tdIn.textContent = fmtInt(ch.in_degree);
            tr.appendChild(tdIn);

            /* first seen (DB registration timestamp; full datetime on hover) */
            var tdSeen = document.createElement("td");
            tdSeen.className = "bo-td--date";
    ...
    function renderLabelsCell(td, ch) {
        ...
        if (!labels.length) {
            ...
            un.className = "bo-org-unassigned";
            un.textContent = "— unassigned";
            td.appendChild(un);
            return;
        }
        var wrap = document.createElement("div");
        wrap.className = "bo-labels-cell";
        labels.forEach(function(l) {
            var chip = document.createElement("span");
            chip.className = "bo-label-chip";
    ...
    function renderSourceChips(td, ch) {
        ...
        (ch.source_ids || []).forEach(function(sid) {
            ...
            rmBtn.title = "Remove from source";
        ...
        if (available.length) {
            ...
            addBtn.className = "bo-chip-add";
            addBtn.innerHTML = "+";
            addBtn.title = "Add to source";
