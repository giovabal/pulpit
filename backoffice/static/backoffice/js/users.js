(function() {
    ...
    function renderRow(user, editing) {
        ...
        if (editing) {
            ...
            eIn.type = "email";
            eIn.className = "bo-input bo-input--wide";
            eIn.value = user.email || "";
            tdE.appendChild(eIn);
            tr.appendChild(tdE);

            var tdSt = document.createElement("td");
            tdSt.className = "bo-td--center";
            var stChk = document.createElement("input");
            stChk.type = "checkbox";
            stChk.checked = user.is_staff;
            tdSt.appendChild(stChk);
            tr.appendChild(tdSt);

            var tdAc = document.createElement("td");
            tdAc.className = "bo-td--center";
            var acChk = document.createElement("input");
            acChk.type = "checkbox";
            acChk.checked = user.is_active;
            tdAc.appendChild(acChk);
            tr.appendChild(tdAc);

            var tdJ = document.createElement("td");
            tdJ.textContent = fmtDate(user.date_joined);
            tr.appendChild(tdJ);

            var tdPw = document.createElement("td");
            var pwIn = document.createElement("input");
            pwIn.type = "password";
            pwIn.className = "bo-input";
            pwIn.placeholder = "New password (optional)";
            ...
            tdPw.appendChild(pwIn);
            tdPw.appendChild(saveBtn);
            tdPw.appendChild(cancelBtn);
            tr.appendChild(tdPw);
        } else {
            var tdEd = document.createElement("td");
            tdEd.textContent = user.email || "—";
            tr.appendChild(tdEd);
            var tdStd = document.createElement("td");
            tdStd.className = "bo-td--center";
            tdStd.appendChild(boolIcon(user.is_staff));
            tr.appendChild(tdStd);
            var tdAcd = document.createElement("td");
            tdAcd.className = "bo-td--center";
            tdAcd.appendChild(boolIcon(user.is_active));
            tr.appendChild(tdAcd);
            var tdJd = document.createElement("td");
            tdJd.textContent = fmtDate(user.date_joined);
            tr.appendChild(tdJd);
    ...
    function loadUsers() {
        apiFetch(API + "?limit=" + BACKOFFICE_PAGE_SIZE + "&offset=" + _offset)
            .then(function(data) {
                _total = data.count;
                $tbody.innerHTML = "";
                if (!data.results.length) { $tbody.innerHTML = '<tr><td colspan="5" class="bo-empty">No users found.</td></tr>'; } else { data.results.forEach(function(u) { $tbody.appendChild(renderRow(u, false)); }); }
                _renderPagination();
            }).catch(function(e) { showToast("Error: " + e.message, "error"); });
    }
