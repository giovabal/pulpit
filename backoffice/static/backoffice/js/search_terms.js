(function() {
    ...
    function renderRow(term) {
        ...
        tdW.style.fontFamily = "var(--font-mono)";
        tdW.textContent = term.word;
        tr.appendChild(tdW);

        var tdD = document.createElement("td");
        tdD.className = "text-muted";
        tdD.style.fontSize = ".875rem";
        tdD.textContent = term.last_check ? fmtDate(term.last_check) : "—";
        tr.appendChild(tdD);
    ...
    function loadTerms() {
        apiFetch(API + "?limit=" + BACKOFFICE_PAGE_SIZE + "&offset=" + _offset)
            .then(function(data) {
                _total = data.count;
                $tbody.innerHTML = "";
                if (!data.results.length) { $tbody.innerHTML = '<tr><td colspan="3" class="bo-empty">No search terms yet.</td></tr>'; } else { data.results.forEach(function(t) { $tbody.appendChild(renderRow(t)); }); }
                _renderPagination();
            }).catch(function(e) { showToast("Error: " + e.message, "error"); });
    }
