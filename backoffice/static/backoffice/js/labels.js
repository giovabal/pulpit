(function() {
    ...
    function renderGroupCard(group) {
        ...
        card.className = "bo-label-group-card";
        card.appendChild(groupHeadView(card, group));

        var wrap = document.createElement("div");
        wrap.className = "table-responsive";
        var table = document.createElement("table");
        table.className = "bo-table bo-label-table";
    ...
    function groupHeadView(card, group) {
        ...
        head.className = "bo-label-group-head";

        var dot = document.createElement("span");
        dot.className = "bo-org-dot";
        dot.style.background = group.color || "#ccc";
        var name = document.createElement("span");
        name.className = "bo-label-group-name fw-semibold";
        ...
        cnt.className = "text-muted small ms-2";
        cnt.textContent = fmtInt(group.label_count) + " label" + (group.label_count !== 1 ? "s" : "");
        head.appendChild(cnt);

        var actions = document.createElement("span");
        actions.className = "bo-label-group-actions ms-auto";
        var addLabelBtn = document.createElement("button");
        addLabelBtn.className = "bo-btn bo-btn--sm bo-btn--ghost";
        ...
        var recolorBtn = document.createElement("button");
        recolorBtn.className = "bo-btn bo-btn--sm bo-btn--ghost";
        ...
        actions.appendChild(recolorBtn);
    ...
    function groupHeadEdit(card, group) {
        ...
        color.type = "color";
        color.className = "bo-input bo-input--color";
        color.value = group.color || "#4338ca";
        var name = document.createElement("input");
        name.className = "bo-input bo-input--wide";
        name.value = group.name;
        var partLbl = document.createElement("label");
        partLbl.className = "bo-check-label";
        var part = document.createElement("input");
        part.type = "checkbox";
        part.checked = group.is_partition;
        partLbl.appendChild(part);
        partLbl.appendChild(document.createTextNode(" Partition"));
        var primLbl = document.createElement("label");
        primLbl.className = "bo-check-label";
        var prim = document.createElement("input");
        prim.type = "checkbox";
        prim.checked = group.is_primary;
        primLbl.appendChild(prim);
        primLbl.appendChild(document.createTextNode(" Primary"));
        var contLbl = document.createElement("label");
        contLbl.className = "bo-check-label";
        var cont = document.createElement("input");
        cont.type = "checkbox";
        cont.checked = group.is_container;
        contLbl.appendChild(cont);
        contLbl.appendChild(document.createTextNode(" Container"));

        var save = document.createElement("button");
        save.className = "bo-btn bo-btn--sm ms-auto";
        save.textContent = "Save";
        var cancel = document.createElement("button");
        cancel.className = "bo-btn bo-btn--sm bo-btn--ghost";
        ...
        save.addEventListener("click", function() {
            apiFetch(API_GROUPS + group.id + "/", {
                    ...
                    body: {
                        ...
                        color: color.value,
                        is_partition: part.checked,
                        is_primary: prim.checked,
                        is_container: cont.checked,
                    },
                }).then(function() {
        ...
        head.appendChild(name);
        head.appendChild(partLbl);
        head.appendChild(primLbl);
        head.appendChild(contLbl);
    ...
    function labelRowView(tbody, group, label) {
        ...
        var dot = document.createElement("span");
        dot.className = "bo-org-dot";
        dot.style.background = label.color || "#ccc";
        tdC.appendChild(dot);
        tr.appendChild(tdC);

        var tdN = document.createElement("td");
        tdN.textContent = label.name;
        tr.appendChild(tdN);
        ...
        if (group.is_container) {
            ...
            tdCnt.className = "bo-td--num";
            tdCnt.textContent = fmtInt(label.channel_count);
    ...
    function labelRowEdit(tbody, group, label) {
        ...
        tdC.appendChild(color);
        tr.appendChild(tdC);

        var tdN = document.createElement("td");
        var name = document.createElement("input");
        name.className = "bo-input bo-input--wide";
        name.value = label ? label.name : "";
        ...
        if (group.is_container) {
            ...
            inTarget.type = "checkbox";
            inTarget.checked = label ? label.is_in_target : false;
            tdI.appendChild(inTarget);
            tr.appendChild(tdI);

            var tdCnt = document.createElement("td");
            tdCnt.className = "bo-td--num";
            tdCnt.textContent = label ? fmtInt(label.channel_count) : "—";
            tr.appendChild(tdCnt);
        }

        var tdA = document.createElement("td");
        var save = document.createElement("button");
        save.className = "bo-btn bo-btn--sm";
        save.textContent = "Save";
        var cancel = document.createElement("button");
        cancel.className = "bo-btn bo-btn--sm bo-btn--ghost";
        cancel.textContent = "Cancel";
        save.addEventListener("click", function() {
            ...
            req.then(function() {
                    showToast(label ? "Label saved." : "Label created.");
                    loadAll();
                })
                .catch(function(e) { showToast("Error: " + e.message, "error"); });
        });
    ...
    function childChip(group, link) {
        ...
        nameBtn.type = "button";
        nameBtn.className = "bo-chip-name-btn";
        nameBtn.textContent = link.label_name;
        ...
        rmBtn.type = "button";
        rmBtn.className = "bo-chip-remove";
    ...
    function openParentDropdown(anchor, group, labelId, link) {
        ...
        dropdown.className = "bo-chip-dropdown";
    ...
    function childPaletteView(group) {
        var wrap = document.createElement("div");
        wrap.className = "bo-child-palette";

        var bar = document.createElement("div");
        bar.className = "bo-child-palette-bar";
        var lbl = document.createElement("label");
        lbl.className = "text-muted small";
        lbl.textContent = "Assign labels from:";
        var select = document.createElement("select");
        select.className = "bo-select bo-select--sm";
        ...
        chips.className = "bo-child-palette-chips";
    ...
    function assignChild(parentLabel, labelId) {
        ...
        apiFetch(API_PARENTS, { method: "POST", body: { parent_id: parentLabel.id, label_id: labelId } })
            .then(function(lk) {
                showToast('"' + lk.label_name + '" → ' + parentLabel.name + ".");
                loadAll();
            })
            .catch(function(e) { showToast("Error: " + e.message, "error"); });
    }
    ...
    function unassignChild(link) {
        apiFetch(API_PARENTS + link.id + "/", { method: "DELETE" })
            .then(function() {
                showToast('"' + link.label_name + '" unassigned.');
                loadAll();
            })
            .catch(function(e) { showToast("Error: " + e.message, "error"); });
    }
    ...
    function buildRecolorModal() {
        ...
        _recolorEl.className = "modal fade";
        _recolorEl.id = "recolor-modal";
        _recolorEl.tabIndex = -1;
        _recolorEl.setAttribute("aria-hidden", "true");
    ...
    function openRecolor(group) {
        ...
        if (!hasLabels) {
            ...
            _recolorLabels.forEach(function(label) {
                ...
                chip.className = "bo-recolor-chip";
                var input = document.createElement("input");
                input.type = "color";
                input.className = "bo-input bo-input--color";
                input.value = label.color || "#cccccc";
                input.setAttribute("aria-label", "Colour for " + label.name);
                var name = document.createElement("span");
                name.className = "bo-recolor-chip-name";
