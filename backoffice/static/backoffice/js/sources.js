(function() {
    ...
    function renderRow(src, editing) {
        ...
        if (editing) {
            ...
            descInput.className = "bo-input bo-input--wide bo-input--full";
            descInput.rows = 4;
            descInput.value = src.description || "";
            tdD.appendChild(descInput);
            tr.appendChild(tdD);

            var tdNo = document.createElement("td");
            var noteInput = document.createElement("textarea");
            noteInput.className = "bo-input bo-input--wide bo-input--full";
            noteInput.rows = 4;
            noteInput.value = src.note || "";
            tdNo.appendChild(noteInput);
            tr.appendChild(tdNo);

            var tdCnt = document.createElement("td");
            tdCnt.className = "bo-td--num";
            tdCnt.textContent = fmtInt(src.channel_count);
            ...
            tdNd.textContent = src.name;
            tr.appendChild(tdNd);
            var tdDd = document.createElement("td");
            tdDd.className = "text-muted";
            tdDd.style.fontSize = ".875rem";
            tdDd.textContent = src.description || "";
            tr.appendChild(tdDd);
            var tdNod = document.createElement("td");
            tdNod.className = "text-muted";
            tdNod.style.fontSize = ".875rem";
            tdNod.textContent = src.note || "";
            tr.appendChild(tdNod);
            var tdCd = document.createElement("td");
            tdCd.className = "bo-td--num";
            tdCd.textContent = fmtInt(src.channel_count);
            tr.appendChild(tdCd);
    ...
    function loadSources() {
        apiFetch(API + "?limit=" + BACKOFFICE_PAGE_SIZE + "&offset=" + _offset)
            .then(function(data) {
                _total = data.count;
                $tbody.innerHTML = "";
                if (!data.results.length) { $tbody.innerHTML = '<tr><td colspan="5" class="bo-empty">No sources yet.</td></tr>'; } else { data.results.forEach(function(s) { $tbody.appendChild(renderRow(s, false)); }); }
                _renderPagination();
            }).catch(function(e) { showToast("Error: " + e.message, "error"); });
    }
